...
/**
 * Created by toroptsev on 22.09.16.
 */
...
//            chain.doFilter(request, response);
            ...
            logger.error("Failed to do filter", a);
        }
    }
        ...
        if (!multipartFormData) {
            ...
            if (StringUtils.isNotEmpty(requestBody)) {
                ...
                if (requestBody.length() > 15000) {
                    logger.debug("Long request body for UUID[" + uuid + "]: " + requestBody);
    ...
    private void logResponse(HttpServletRequest httpServletRequest, BufferedResponseWrapper bufferedResponse, String uuid, long timeElapsed) {
        ...
        if (printContent) {
            ...
            if (responseBody.length() > 15000) {
                logger.debug("Long response body for UUID[" + uuid + "]: " + responseBody);
